...
binary_save_path   = "results/4node_lag2_lambda_sweep_binary_scores.npy"
strength_save_path = "results/4node_lag2_lambda_sweep_strength_scores.npy"
lambda_save_path   = "results/4node_lag2_lambdas.npy"
...
def worker_fn(worker_id, sim_start_idx, sim_end_idx, X_test_np, lambdas, device_str):
    ...
    np.save(f"results/worker/4node_lag2_worker{worker_id}_binary.npy",   results_binary)
    np.save(f"results/worker/4node_lag2_worker{worker_id}_strength.npy", results_strength)
...
if __name__ == "__main__":
    ...
    for wid, (s, e) in enumerate(ranges):
        all_binary[:, s:e]   = np.load(f"results/worker/4node_lag2_worker{wid}_binary.npy")
        all_strength[:, s:e] = np.load(f"results/worker/4node_lag2_worker{wid}_strength.npy")
