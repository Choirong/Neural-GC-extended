...
# ── 1. 데이터 로드 ──────────────────────────────────────────────
lambdas       = np.load("results/4node_lag2_lambdas.npy")
binary_scores = np.load("results/4node_lag2_lambda_sweep_binary_scores.npy")

with h5py.File("simulated/260420_data/four_nodes/result.mat", "r") as f:
...
fig.suptitle("Neural-GC Baseline — 4-node, lag=2\nROC Curve (λ sweep)", fontsize=13)
...
plt.tight_layout()
out_path = "results/roc_4node_lag2.png"
